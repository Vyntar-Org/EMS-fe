...
const API_BASE_URL =
  process.env.REACT_APP_API_URL || "https://bms.api.v1.vyntar.in/api";
...
export const getTemperatureLogs = async (
  ...
  try {
    ...
    console.log(
      `Making temperature logs API call for slave ${slaveId} with token:`,
...
export const getTemperatureSlaves = async () => {
  try {
    ...
    console.log(
      "Making temperature slaves API call with token:",
      validToken.substring(0, 20) + "...",
    );
    console.log("API Base URL:", apiClient.defaults.baseURL);
 ...
 */
export const getTemperatureLogsWithNames = async (
  slaveId,
  startDatetime,
  endDatetime,
  limit = 30,
  ...
  try {
    ...
    let slavesMap = {};
    if (
      slavesData.success === true &&
      slavesData.data &&
      slavesData.data.slaves
    ) {
