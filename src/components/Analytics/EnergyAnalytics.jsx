...
const GlobalFiltersRow = memo(({ dateTime, onDateChange, addNewComparisonRow }) => (
	<Box
		sx={{
			pb: 2,
...
const AnalyticsRow = memo(({
	...
	const performanceChartOptions = useMemo(() => ({
		...
		legend: { show: false },
		grid: { borderColor: '#e0e0e0', strokeDasharray: 0, padding: { top: 0, right: 0, bottom: 0, left: 0 } },
		states: {
	...
	const deviceLabel =
		payload?.slave_id?.label || `Device Segment ${id}`;
				...
				>
					...
					>
						{hoveredData.values.map((item, idx) => {
							...
							const chipText = isDark ? lighten(base, 0.35) : darken(base, 0.25);
...
const EnergyAnalytics = () => {
	...
	const handleSearch = useCallback(async (id) => {
		setPayloads((prevPayloads) => {
			...
			if (!currentPayload?.slave_id || !currentPayload?.parameters?.length) return prevPayloads;
			...
			>
				{rowIds.map((id, index) => {
					const filteredSlaveOptions = slaveOptions.filter(
						(option) => !usedDeviceIds.includes(option.value) || payloads[id]?.slave_id?.value === option.value
					);
