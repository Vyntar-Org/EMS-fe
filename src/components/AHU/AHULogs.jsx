...
const AHULogsFilterHeader = ({
	...
	const downloadApiUrl = useMemo(() => {
		return LogUrlBuilder('AHU_LOGS_DATA').build({
			payload,
			isDownload: true,
		});
	}, [payload]);
...
const AHULogs = () => {
	...
	const handleSearch = async (paginationDetails = apiPaginationParams) => {
		...
		setLoading(true);
		try {
			const url = LogUrlBuilder('AHU_LOGS_DATA').build({
				payload,
			...
			console.error('AHU logs API error:', error);
		} finally {
			setLoading(false);
		}
	};
