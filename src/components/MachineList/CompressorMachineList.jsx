...
import { DownloadForOffline, Insights, BarChart, TableChart } from "@mui/icons-material";
...
const CompressorMetricBlock = ({ machine, handleOpenModal, onStoppageClick }) => {
  const latest = machine?.latest || {};
  const rawStatus = machine?.current_status?.status || machine?.status || latest?.status || "Unknown";
  const status = String(rawStatus);
  const isOnline = status?.toLowerCase() === "online";

  const lastUpdated =
    machine?.current_status?.last_updated || latest?.last_ts || machine?.last_updated || machine?.last_ts;

  const statusFrom =
    machine?.current_status?.since_display || machine?.current_status?.status_from || latest?.status_from || latest?.status_since || latest?.from || machine?.status_from || "";

  const name =
    machine?.name || machine?.slave_name || machine?.device_uid || machine?.compressor_no || "Unnamed Device";

  const lastStoppageStart =
    machine?.last_stoppage?.start_time || machine?.last_stoppage?.start || latest?.stop_start || latest?.last_stop_start;

  const lastStoppageEnd =
    machine?.last_stoppage?.end_time || machine?.last_stoppage?.end || latest?.stop_end || latest?.last_stop_end;

  const lastStoppageDuration =
    machine?.last_stoppage?.duration || latest?.stop_duration || latest?.last_stop_duration || machine?.last_stoppage?.duration_display || "N/A";

  const previous8Count =
    machine?.history_8h?.count ?? machine?.previous_8hrs_stoppages ?? machine?.stoppages_previous_8hrs ?? machine?.stoppages_8hrs ?? 0;

  const previous24Count =
    machine?.history_24h?.count ?? machine?.previous_24hrs_stoppages ?? machine?.stoppages_previous_24hrs ?? machine?.stoppages_24hrs ?? 0;

  const previous8Duration =
    machine?.history_8h?.total_duration ?? machine?.previous_8hrs_duration ?? machine?.stoppage_duration_previous_8hrs ?? machine?.stoppage_duration_8hrs ?? "N/A";

  const previous24Duration =
    machine?.history_24h?.total_duration ?? machine?.previous_24hrs_duration ?? machine?.stoppage_duration_previous_24hrs ?? machine?.stoppage_duration_24hrs ?? "N/A";
  ...
  return (
    ...
    >
      ...
      <Box sx={{ mb: 1 }}>
        <ResponsiveTextWrapper value="Last Stoppage" fontWeight="bold" fontSize="14px" />
        <Table size="small" sx={{ mt: 1, width: "100%", tableLayout: "fixed" }}>
          <TableHead>
            <TableRow>
              <TableCell sx={{ fontWeight: "bold", border: 0, p: 0.5, fontSize: "13px" }}>Start Time</TableCell>
              <TableCell align="center" sx={{ fontWeight: "bold", border: 0, p: 0.5, fontSize: "13px" }}>End Time</TableCell>
              <TableCell align="center" sx={{ fontWeight: "bold", border: 0, p: 0.5, fontSize: "13px" }}>Duration</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            <TableRow>
              ...
              <TableCell align="center" sx={{ border: 0, p: 0.5, fontSize: "12px" }}>
                {lastStoppageEnd || "-"}
              </TableCell>
              <TableCell align="center" sx={{ border: 0, p: 0.5, fontSize: "12px" }}>
      ...
      <Box sx={{ mb: 1 }}>
        <Table size="small" sx={{ mt: 1, width: "100%", tableLayout: "fixed" }}>
          <TableHead>
            <TableRow>
              <TableCell align="" sx={{ fontWeight: "bold", border: 0, p: 0.5, fontSize: "13px" }}>Stoppages History</TableCell>
              <TableCell align="center" sx={{ fontWeight: "bold", border: 0, p: 0.5, fontSize: "13px" }}>Previous 8hrs</TableCell>
              <TableCell align="center" sx={{ fontWeight: "bold", border: 0, p: 0.5, fontSize: "13px" }}>Previous 24hrs</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            <TableRow>
              <TableCell sx={{ border: 0, p: 0.5, fontSize: "12px", fontWeight: "bold" }}>No. of Stoppages</TableCell>
              <TableCell align="center" sx={{ border: 0, p: 0.5, fontSize: "12px", fontWeight: "bold" }}>
                <Box
                  component="span"
                  onClick={() => previous8Count > 0 && onStoppageClick(machine, 8)}
                  sx={{
                    cursor: previous8Count > 0 ? 'pointer' : 'default',
                    color: previous8Count > 0 ? '#2F6FB0' : 'inherit',
                    fontWeight: 600,
                    '&:hover': previous8Count > 0 ? { textDecoration: 'underline' } : {},
                  }}
                >
                  {previous8Count}
                </Box>
              ...
              <TableCell align="center" sx={{ border: 0, p: 0.5, fontSize: "12px", fontWeight: "bold" }}>
                 <Box
                  component="span"
                  onClick={() => previous24Count > 0 && onStoppageClick(machine, 24)}
                  sx={{
                    cursor: previous24Count > 0 ? 'pointer' : 'default',
                    color: previous24Count > 0 ? '#2F6FB0' : 'inherit',
                    fontWeight: 600,
                    '&:hover': previous24Count > 0 ? { textDecoration: 'underline' } : {},
                  }}
                >
                  {previous24Count}
                </Box>
            ...
            <TableRow>
              <TableCell sx={{ border: 0, p: 0.5, fontSize: "12px", fontWeight: "bold"  }}>Stoppage Duration</TableCell>
              <TableCell align="center" sx={{ border: 0, p: 0.5, fontSize: "12px" }}>{previous8Duration}</TableCell>
              <TableCell align="center" sx={{ border: 0, p: 0.5, fontSize: "12px" }}>{previous24Duration}</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </Box>
...
const handleDownload = (filteredMachines, selectedApp) => {
  ...
  const rows = filteredMachines.map((machine) => {
    const latest = machine?.latest || {};
    const status = machine?.current_status?.status || machine?.status || latest?.status || "N/A";
    const since =
      machine?.current_status?.since_display || machine?.current_status?.status_from || latest?.status_from || latest?.status_since || machine?.status_from || "N/A";
    const lastUpdated =
      machine?.current_status?.last_updated || latest?.last_ts || machine?.last_updated || machine?.last_ts;

    const lastStoppageStart = machine?.last_stoppage?.start_time || machine?.last_stoppage?.start || latest?.stop_start || latest?.last_stop_start || "-";
    const lastStoppageEnd = machine?.last_stoppage?.end_time || machine?.last_stoppage?.end || latest?.stop_end || latest?.last_stop_end || "-";
    const lastStoppageDuration = machine?.last_stoppage?.duration || latest?.stop_duration || latest?.last_stop_duration || machine?.last_stoppage?.duration_display || "-";

    const count8h = machine?.history_8h?.count ?? machine?.previous_8hrs_stoppages ?? machine?.stoppages_previous_8hrs ?? machine?.stoppages_8hrs ?? 0;
    const duration8h = (machine?.history_8h?.total_duration ?? machine?.previous_8hrs_duration ?? machine?.stoppage_duration_previous_8hrs ?? machine?.stoppage_duration_8hrs) || "-";
    const count24h = machine?.history_24h?.count ?? machine?.previous_24hrs_stoppages ?? machine?.stoppages_previous_24hrs ?? machine?.stoppages_24hrs ?? 0;
    const duration24h = (machine?.history_24h?.total_duration ?? machine?.previous_24hrs_duration ?? machine?.stoppage_duration_previous_24hrs ?? machine?.stoppage_duration_24hrs) || "-";

    return [
      machine?.name || machine?.slave_name || machine?.compressor_no || machine?.device_uid || "N/A",
...
const StoppageHistoryModal = ({ open, onClose, machine, hours }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await api.get(API_URLS.COMPRESSOR_DOWNTIME_HISTORY(slaveId, hours));
      ...
      if (res?.data?.periods) {
        setStoppages(res.data.periods.map(p => ({
  ...
  const chartOptions = useMemo(() => ({
    ...
    xaxis: {
      ...
      min: windowRange?.from ? parseFlexibleDate(windowRange.from) : undefined,
      max: windowRange?.to ? parseFlexibleDate(windowRange.to) : undefined,
    },
    yaxis: {
      title: { text: 'Status', style: { color: '#6B7280', fontSize: '12px' } },
      min: -0.1,
      max: 1.1,
      tickAmount: 2,
      labels: {
  ...
  const handleTableDownload = () => {
    ...
    const totalDuration = summary?.total_downtime || formatMinutesToDuration(Math.round(stoppages.reduce((sum, item) => sum + (item.durationSeconds || 0), 0) / 60));
    rows.push(['', '', '', '']);
    ...
    >
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 2 }}>
        <Tabs value={activeTab} onChange={(e, v) => setActiveTab(v)}>
          <Tab icon={<BarChart sx={{ fontSize: 18 }} />} iconPosition="start" label="Chart" />
          <Tab icon={<TableChart sx={{ fontSize: 18 }} />} iconPosition="start" label="Data" />
        </Tabs>
      </Box>
          ...
          {activeTab === 1 && (
            <Box>
               <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 1 }}>
                 <Button size="small" startIcon={<DownloadForOffline />} onClick={handleTableDownload} disabled={stoppages.length === 0}>
                   Download CSV
                 </Button>
               </Box>
               {stoppages.length > 0 ? (
                   ...
                   <Table size="small" stickyHeader>
                     ...
                     <TableBody>
                       {stoppages.map((item, idx) => (
                         <TableRow key={idx} sx={{ bgcolor: item.isOngoing ? '#fff5f5' : 'inherit' }}>
                           <TableCell>{idx + 1}</TableCell>
                           <TableCell sx={{ fontFamily: 'monospace' }}>{item.start_time || '-'}</TableCell>
                           <TableCell sx={{ fontFamily: 'monospace', color: item.isOngoing ? 'error.main' : 'text.primary', fontWeight: item.isOngoing ? 'bold' : 'normal' }}>
                             {item.isOngoing ? 'Ongoing' : (item.end_time || '-')}
                           </TableCell>
                           <TableCell sx={{ fontFamily: 'monospace' }}>{item.duration || '-'}</TableCell>
                         </TableRow>
                       ))}
                     </TableBody>
                   </Table>
               ...
               {stoppages.length > 0 && (
                 <Box sx={{ mt: 2, p: 1.5, bgcolor: '#f5f5f5', borderRadius: 1, display: 'flex', justifyContent: 'space-between' }}>
                   <Typography variant="body2">Total Stoppages: <strong>{summary?.count ?? stoppages.length}</strong></Typography>
                   <Typography variant="body2">Total Duration: <strong>{summary?.total_downtime ?? formatMinutesToDuration(Math.round(stoppages.reduce((sum, i) => sum + (i.durationSeconds || 0), 0) / 60))}</strong></Typography>
                 </Box>
               )}
            </Box>
          )}
...
const ModalContentForTrend = ({ handleTabChange, tab, tabDesc, slaveId, slaveName, trendOptions }) => {
  ...
  const chartSeries = useMemo(() => {
    ...
    if (isStatusTrend) {
      ...
      chartData.forEach((item) => {
        ...
        const isOnline = rawStatus === 1 || rawStatus === "1" || String(rawStatus).toLowerCase() === "online" || rawStatus === true;
  ...
  const chartOptions = useMemo(() => {
    ...
    if (isStatusTrend) {
      ...
      const chartCategories = chartData.map((item) => {
        const timestamp = item.start || item.timestamp || item.time || item.date;
        if (!timestamp) return "";
...
const CompressorMachineList = () => {
  ...
  const [stoppageModal, setStoppageModal] = useState({ open: false, machine: null, hours: 8 });
  ...
  const handleTabChange = (value) => {
    const selectedOption = trendOptions.find((option) => option.value === value) || defaultTrend;
  ...
  return (
    <>
            ...
            {isLoading ? (
              ...
              <Grid container rowGap={1} columnSpacing={1}>
                {filteredMachines.map((machine) => (
                  <Grid item xs={12} sm={6} md={4} key={`compressor-machine-${machine?.slave_id || machine?.id}`}>
      ...
      <PremiumModal
        ...
        title={`${modalDetails?.data?.name || modalDetails?.data?.slave_name || "Compressor"} - ${modalDetails?.tabDesc || "Trend"}`}
        confirmText={null}
        cancelText={null}
      >
        {Boolean(modalDetails?.isOpen) ? (
          <ModalContentForTrend
            ...
            slaveName={modalDetails?.data?.name || modalDetails?.data?.device_uid}
      ...
      <StoppageHistoryModal 
        ...
        onClose={() => setStoppageModal({ open: false, machine: null, hours: 8 })}
