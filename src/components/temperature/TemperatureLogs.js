...
function TemperatureLogs({ onSidebarToggle, sidebarVisible }) {
    ...
    const fetchDevices = async () => {
      try {
        ...
        if (slavesData.success && slavesData.data && slavesData.data.slaves) {
          ...
          const deviceList = [
            "all",
            ...slavesData.data.slaves.map((slave) => slave.slave_name),
          ];
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (
        slavesData.success &&
        slavesData.data &&
        Array.isArray(slavesData.data.slaves)
      ) {
      ...
      if (
        logsData.success &&
        logsData.data &&
        Array.isArray(logsData.data.logs)
      ) {
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    return (
      ...
      (log.slave_name &&
        log.slave_name.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  const shouldShowPagination =
    searchClicked && totalRecords > 0 && totalRecords > rowsPerPage;
  ...
  const isAllParametersSelected =
    selectedColumn.length === allParameterValues.length;
  ...
  const exportToExcel = async () => {
    ...
    if (slavesData.success && Array.isArray(slavesData.data?.slaves)) {
      const selectedSlave = slavesData.data.slaves.find(
        (s) => s.slave_name === filterDevice,
      );
      if (selectedSlave) slaveId = selectedSlave.slave_id;
    }
        ...
        <CardContent>
          ...
          <Box className="logs-header">
            ...
            >
                      ...
                      {isAllParametersSelected ? (
                        ...
                        selected.slice(0, 2).map((value) => (
                          <Chip
                            key={value}
                            label={
                              allParameters.find((p) => p.val === value)
                ...
                >
                  ...
                  <MenuItem
                    ...
                    sx={{
                      ...
                      backgroundColor: isAllParametersSelected
                  ...
                  >
                    <Checkbox
                      ...
                      indeterminate={
                        selectedColumn.length > 0 && !isAllParametersSelected
                      }
                      sx={{
                        p: 0.5,
                        mr: 0.5,
              ...
              >
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DateTimePicker
                    ...
                    value={
                      dayjs.isDayjs(filterStartDate) ? filterStartDate : null
                    }
              ...
              <Button
                ...
                startIcon={<FileDownload sx={{ marginLeft: "12px" }} />}
                sx={{
                  bgcolor: "#217346",
                  color: "#fff",
                  borderRadius: "8px",
              ...
              >
                  ...
                  <TableRow className="log-table-header">
                    {selectedColumn.length > 0 ? (
                      selectedColumn.map((col) => (
                        ...
                        >
                          {allParameters.find((p) => p.val === col)?.label ||
                            col.replace(/_/g, " ")}
                        </TableCell>
                      ))
                    ) : (
                ...
                <TableBody>
                  {paginatedLogs.length > 0 ? (
                    ...
                    <TableRow>
                      <TableCell
                        colSpan={selectedColumn ? selectedColumn.length : 4}
                        align="center"
                      >
                        {paginatedLogs.length === 0
          ...
          {shouldShowPagination && (
            ...
            >
              ...
              >
                Showing {(paginationMeta.offset || 0) + 1} to{" "}
                {Math.min(
                  (paginationMeta.offset || 0) +
                    (paginationMeta.limit || rowsPerPage),
