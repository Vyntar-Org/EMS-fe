...
import {
  getStpMachineList,
  getStpMachineListTrend,
} from "../../auth/stp/StpMachineListApi"; // Adjust path as needed
...
const StpMachineList = () => {
  ...
  const handleDownload = () => {
    ...
    const metaRow = [
      `Device: ${plantData.device_name || "STP"}`,
    ...
    link.setAttribute(
      "download",
      `STP_Machines_${new Date().toISOString().slice(0, 10)}.csv`,
    );
  ...
  const handleTrendClick = async (slave_id, parameter) => {
    ...
    try {
      const response = await getStpMachineListTrend({
        slave_id,
